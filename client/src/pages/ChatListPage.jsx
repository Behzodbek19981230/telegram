...
export function ChatListPage() {
  ...
  const liveChats = useMemo(
    () =>
      chats.map((c) =>
        c.type === 'DIRECT' ? { ...c, otherUser: withPresence(c.otherUser, presenceMap) } : c
      ),
    [chats, presenceMap]
  );

  const unreadChatsCount = useMemo(
    () => liveChats.filter((c) => c.unreadCount > 0).length,
    [liveChats]
  );
      ...
      <header className="chatlist-header">
        <div className="chatlist-header__top">
          ...
          <div className="chatlist-header__actions">
            ...
            <button className="icon-button" onClick={() => setIsSearchOpen((v) => !v)} aria-label="Qidirish">
        ...
        {isSearchOpen && (
          <div className="chatlist-search">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Qidirish" className="chatlist-search__input" />
          </div>
        )}
      </header>
